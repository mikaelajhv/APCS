...
public class SevenSegment extends JPanel implements ActionListener
{
  ...
  public void paintComponent(Graphics g)
  {
  	 ...
  	 segments[0] = segments[0] = number[3] || number[1] || (number[2] && number[0]) || (!number[2] && !number[0]);
  	  
  	 segments[1] = number[3] || number[1] && number[0] || !number[3] && !number[2] || !number[0] && !number[1];
  	  
  	 segments[2] = number[0] || !number[1] || number[2];
  	  
  	 segments[3] = !number[2] && number[1] || !number[0] && number[1] || number[3] || !number[0] && !number[2] || number[0] && number[2] && !number[1];
  			  
  	 segments[4] = !number[0] && (number[1] || !number[2]);
  	  
  	 segments[5] = segments[5] =number[3] || number[2] && (!number[1] || !number[0]) || (!number[1] && !number[0]);
  			  
  	 segments[6] = segments[6] = number[3] || number[1] && (!number[0] || !number[2]) || (number[2] && !number[1]) ;
