...
public class Elevator extends JPanel implements ActionListener
{
  ...
  public void setFloor(int f) {
  	...
  	// Problem 4:
  	/*
  	doorOpen = (buttons[0] && floorBits[0] && buttons[0] && floorBits[1] && buttons[2]) || 
  			(buttons[0] && floorBits[0] && !buttons[0] && floorBits[1] && buttons[0]) || 
  			(!buttons[0] && floorBits[0] && buttons[0] && floorBits[1] && buttons[2]); 
  	
  	doorOpen = (floorBits[0] && floorBits[1] && buttons[2]) 
  			|| (floorBits[0] && !floorBits[0] && buttons[2]) 
  			|| (!floorBits[0] && floorBits[1] && buttons[1]); */
  	...
  	// Problem 5:
  	direction[0] = !floorBits[1] && (!buttons[0] && (buttons[2] || buttons[1])) 
  			|| (!buttons[1] && buttons[2] && !floorBits[0]);

    direction[1] = !buttons[0] && ((!floorBits[1] && buttons[2]) || (floorBits[0] && buttons[1] && !buttons[2])) 
    		|| (floorBits[1] && ((floorBits[0] && buttons[0] && !buttons[2]) || (!floorBits[0] && ((buttons[0] && !buttons[1]) || (!buttons[1] && buttons[2])))));
  

  	// Problem 6:
  	lightUp = (floorBits[1] && buttons[2] && !floorBits[0]) || (!floorBits[1] && floorBits[0]);
  	lightDown = (!floorBits[1] || !buttons[2] || floorBits[0]) && (floorBits[1] || !floorBits[0]);
